...
from coarsening.graph_utils import *
...
def coarsen(G, K=10, r=0.5, max_levels=20, method='variation_edges', algorithm='greedy', Uk=None, lk=None, max_level_r=0.99):
    ...
    for level in range(1,max_levels+1): 
        ...
        Ac_row = get_rows_coarse_matrix(Ac_row, coarsening_list)
...
def get_rows_coarse_matrix(Ac, partitioning):
    """
    This function should be called in order to build the coarsening matrix C. 
    
    Parameters
    ----------
    Ac : the previous graph  coarsened
    partitioning : a list of subgraphs to be contracted
    
    Returns
    -------
    Ac : the new coarsening matrix
    ...
    Ac.rows = np.delete(Ac.rows, rows_to_delete)
    Ac.data = np.delete(Ac.data, rows_to_delete)
    Ac._shape = (N - len(rows_to_delete), N)
...
def contract_variation_linear(G, A=None, K=10, r=0.5, mode='neighborhood'):
    ...
    def subgraph_cost(nodes):
        ...
        return np.linalg.norm(B.T @ L @ B)/(nc)
